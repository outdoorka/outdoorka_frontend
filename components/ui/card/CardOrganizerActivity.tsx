...
import { Button, Fab, Box, Typography, Grid, Paper, CardMedia } from "@mui/material";
...
function CardOrganizerActivity({ isFinish, isPublish, activity }: CardActivityProps) {
		...
		>
			<Box sx={{ position: "relative" }}>
				...
				{!isFinish && (
					<Grid container sx={{
						...
						<Grid item>
							<Fab size="small" color="primary" aria-label={isPublish === 0? "刪除活動": "取消活動"}>
								<ClearIcon />
							</Fab>
						</Grid>
					</Grid>
				...
				<Grid container sx={{
					...
					{!isFinish && isPublish === 1 && (
						<Grid item>
							<Button sx={cardRoundInfoBoxStyle} onClick={() => setDialogOpen(true)}>
			...
			<Box sx={{ px: 4, py: 3  }}>
				<Box sx={cardMiddleContextStyle}>
					...
					<span>{activity?.region} {activity?.city} {activity?.address}</span>
				</Box>
				<Box sx={cardMiddleContextStyle}>
			...
			{!isFinish && (
				<Grid container spacing={1.5} sx={{ pl: 2, pr: 4,  pb: 3  }}>
					...
					<Grid item xs={6}>
						<Button variant="contained" size="small" sx={{width:"100%"}} disabled={isPublish === 1}>編輯</Button>
					</Grid>
					<Grid item xs={12}>
						<Button variant="contained" size="small" sx={{width:"100%"}} disabled={isPublish === 0}>驗票</Button>
					</Grid>
				</Grid>
			)}
		</Paper>
